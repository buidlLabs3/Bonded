...
const std::vector<DefaultSkillSpec>& allDefaultSkillSpecs()
{
    ...
    static const std::vector<DefaultSkillSpec> specs{
        ...
        {"agent.card", "Get or publish a signed A2A Agent Card", {Vault}},
        {"agent.discover", "Discover valid A2A Agent Cards", {Vault}},
        {"agent.task", "Create or complete a paid A2A task", {Vault}},
        {"agent.subscribe", "Read current A2A task state", {Vault}},
        {"agent.cancel", "Cancel and refund an A2A task", {Vault}},
